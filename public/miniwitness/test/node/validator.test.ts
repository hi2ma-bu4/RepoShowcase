...
describe("Square validation", { concurrency: true }, () => {
	test("different colors in same region", () => {
...
describe("Star validation", { concurrency: true }, () => {
	test("pair of stars", () => {
		const puzzle = createBasicGrid(1, 2);
	...
	test("star and square same color", () => {
		const puzzle = createBasicGrid(1, 2);
		puzzle.cells[0][0] = { type: CellType.Star, color: Color.Black };
		puzzle.cells[0][1] = { type: CellType.Square, color: Color.Black };
		const result = core.validateSolution(puzzle, getPath(2));
	...
	test("star and two squares same color", () => {
		const puzzle = createBasicGrid(1, 3);
		puzzle.cells[0][0] = { type: CellType.Star, color: Color.Black };
		puzzle.cells[0][1] = { type: CellType.Square, color: Color.Black };
	...
	test("stars of different colors in same region", () => {
	...
	test("multiple regions with same color stars", () => {
	...
	test("7 stars and 1 square total in puzzle", () => {
	...
	test("two regions, one valid, one invalid", () => {
	...
	test("star outside region with its color marks", () => {
		const puzzle = createBasicGrid(2, 2);
		const path: SolutionPath = {
			points: [
				{ x: 0, y: 2 },
	...
	test("mixed colors with squares", () => {
		const puzzle = createBasicGrid(1, 3);
	...
	test("star with different color square", () => {
		const puzzle = createBasicGrid(1, 2);
	...
	test("star mixed with square of different color", () => {
		const puzzle = createBasicGrid(1, 3);
		// Region is (0,0), (0,1), (0,2)
	...
	test("absent edges as boundaries", () => {
		const puzzle = createBasicGrid(1, 2);
	...
	test("star with multiple same color squares", () => {
		const puzzle = createBasicGrid(1, 4);
		puzzle.cells[0][0] = { type: CellType.Star, color: Color.Black };
		puzzle.cells[0][1] = { type: CellType.Square, color: Color.Black };
		puzzle.cells[0][2] = { type: CellType.Square, color: Color.Black };
	...
	test("color counts are independent", () => {
		const puzzle = createBasicGrid(1, 4);
...
describe("Generator", { concurrency: true }, () => {
	...
	test("Hexagon generation in Rotational grid", () => {
		const generator = new PuzzleGenerator();
		const options = {
...
describe("Difficulty calculation", { concurrency: true }, () => {
	test("simple grid", () => {
		const puzzle = createBasicGrid(2, 2);
	...
	test("more constraints should be harder", () => {
	...
	test("trivial puzzle should have very low difficulty", () => {
	...
	test("sparse 6x6 grid should have very low difficulty", () => {
...
describe("Solution counter", { concurrency: true }, () => {
	test("unique solutions based on region marks", () => {
	...
	test("paths with same mark partitioning", () => {
		const validator = new PuzzleValidator();
		const puzzle = createBasicGrid(2, 2);
	...
	test("2x2 grid with no constraints", () => {
		const validator = new PuzzleValidator();
		const puzzle = createBasicGrid(2, 2);
		const grid = Grid.fromData(puzzle);
		const count = validator.countSolutions(grid);
...
describe("Edge Behavior", { concurrency: true }, () => {
