...
test("Error reporting - Square color conflict", () => {
	...
	assert.ok(result.errorCells && result.errorCells.length >= 2, "Both squares should be in error");
	const errorKeys = new Set(result.errorCells.map((p) => `${p.x},${p.y}`));
	assert.ok(errorKeys.has("0,0"));
	assert.ok(errorKeys.has("1,0"));
});

test("Error reporting - Star single", () => {
...
test("Error reporting - Eraser failing to negate (Star pair + Eraser)", () => {
	...
	// Red Star pair is valid by itself. Eraser has nothing to negate (negating one star would create an error).
	...
	assert.strictEqual(result.isValid, false);
	assert.strictEqual(result.invalidatedCells?.length, 0, "Should not negate anything as it doesn't help");
	assert.ok(
		result.errorCells?.some((p) => p.x === 1 && p.y === 0),
		"Eraser should be in error",
	);
});

test("Error reporting - Eraser in valid region should not negate", () => {
	const puzzle = createBasicGrid(1, 2);
	puzzle.cells[0][0] = { type: CellType.Square, color: Color.Black };
	puzzle.cells[0][1] = { type: CellType.Eraser, color: Color.White };
	// Square(Black) is valid by itself. Eraser has nothing to negate.
	...
	assert.strictEqual(result.isValid, false);
	assert.strictEqual(result.invalidatedCells?.length, 0, "No cells should be invalidated");
	assert.ok(
		result.errorCells?.some((p) => p.x === 1 && p.y === 0),
		"Eraser should be in error",
	);
	assert.ok(!result.errorCells?.some((p) => p.x === 0 && p.y === 0), "Valid square should NOT be in error");
});
