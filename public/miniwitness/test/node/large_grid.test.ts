...
test("Large grid (8x8) generation distribution", () => {
	const options = {
		useHexagons: true,
		useSquares: true,
		useStars: true,
	...
	// We expect some distribution. Given shuffling, it's very unlikely to have 0 in any half if marksCount is high.
	// But we use a loose check to avoid flakiness.
	if (marksCount >= 10) {
...
test("Large grid (8x8) difficulty calculation stability", () => {
	// We just want to ensure it doesn't crash and returns a value.
	// We don't assert > 0 because of the search limit on 8x8.
