...
var BigFloatStream = class _BigFloatStream {
  ...
  harmonicMean() {
    ...
    if (arr.length === 0) throw new TypeError("No elements");
    const p = arr[0] instanceof BigFloat ? arr[0]._precision : arr[0].precision;
    let sumRecip = arr[0] instanceof BigFloat ? new BigFloat(0, p) : new BigFloatComplex(0, 0, p);
  ...
  rms() {
    ...
    if (arr.length === 0) throw new TypeError("No elements");
    const p = arr[0] instanceof BigFloat ? arr[0]._precision : arr[0].precision;
    let sumSq = arr[0] instanceof BigFloat ? new BigFloat(0, p) : new BigFloatComplex(0, 0, p);
    for (const val of arr) {
...
var DimensionMismatchError = class extends BigFloatError {
};
var SingularMatrixError = class extends BigFloatError {
};
...
var BigFloatComplexVector = class _BigFloatComplexVector {
   ...
   */
  static _throwNonRealVectorOperation(operation) {
   ...
   */
  static _coerceRealVector(value, referenceValues, operation) {
    const vector = _BigFloatComplexVector._coerceVector(value, referenceValues);
    return BigFloatVector.from(
      vector._values.map((entry) => {
        if (!entry.isReal()) _BigFloatComplexVector._throwNonRealVectorOperation(operation);
        return entry.real;
      })
    );
  }
   ...
   * @throws {TypeError} 非実数複素数要素を含む場合
   */
  _toRealVector(operation) {
    return BigFloatVector.from(
      this._values.map((value) => {
        if (!value.isReal()) _BigFloatComplexVector._throwNonRealVectorOperation(operation);
        return value.real;
      })
    );
  }
  ...
  angleTo(other) {
    ...
    const right = _BigFloatComplexVector._coerceRealVector(other, this._values, "angleTo");
...
var BigFloatVector = class _BigFloatVector {
  ...
  constructor(values = [], precision) {
    ...
    this._values = array.map((value) => {
      if (BigFloat._isComplexValue(value)) {
        return _BigFloatVector._toBigFloat(value.real, resolvedPrecision);
      }
      return _BigFloatVector._toBigFloat(value, resolvedPrecision);
    });
  }
   ...
   */
  static _coerceVector(value, referenceValues = []) {
    if (value instanceof _BigFloatVector || value instanceof BigFloatComplexVector) return value;
    const array = Array.from(value);
  ...
  _mapValues(fn) {
    const values = this._values.map((value, index) => {
      const mapped = fn(value.clone(), index);
      return _BigFloatVector._toBigFloat(mapped, value._precision);
    });
  ...
  _mapWithOperand(other, fn) {
    ...
    const right = _BigFloatVector._toBigFloat(other, this._values[0]?._precision);
    return this._mapValues((value, index) => fn(value, right, index));
  }
  /**
  ...
  atan2(x) {
    return this._mapWithOperand(x, (left, right) => {
      if (left instanceof BigFloat) return left.atan2(right);
   ...
   */
  Ei() {
    return this._mapValues((value) => value.Ei());
  }
  /**
   * 各要素に対して対数積分 li(x) を計算する
   ...
   */
  li() {
    return this._mapValues((value) => value.li());
  }
  // ====================================================================================================
  // * 統計関数
  // ====================================================================================================
  ...
  variance() {
    if (this.isEmpty()) throw new TypeError("No elements");
    if (this.length === 1) return new BigFloat(0, this._values[0]._precision);
    const mean = this.average();
    let sumSq = new BigFloat(0, this._values[0]._precision);
    for (const val of this._values) {
      const diff = val.sub(mean);
      sumSq = sumSq.add(diff.mul(diff));
    }
  ...
  harmonicMean() {
    if (this.isEmpty()) throw new TypeError("No elements");
    let sumRecip = new BigFloat(0, this._values[0]._precision);
    for (const val of this._values) {
      sumRecip = sumRecip.add(val.reciprocal());
    }
    return new BigFloat(this.length, this._values[0]._precision).div(sumRecip);
  }
  /**
   * 二乗平均平方根 (RMS) を計算する
   * @returns RMS
...
var BigFloatComplex = class _BigFloatComplex {
   ...
   * @throws {TypeError} 常に送出
   */
  static _throwNonRealOperation(operation) {
   ...
   * @throws {RangeError} 精度が 0 未満または MAX_PRECISION を超える場合
   */
  static _fromRealResult(real) {
    return _BigFloatComplex._fromBigFloats(real, new BigFloat(0, real._precision));
  }
  /**
   * @throws {RangeError} 精度が 0 未満または MAX_PRECISION を超える場合
   * @throws {SyntaxError} 文字列が複素数表現として無効な場合
   ...
   * @throws {TypeError} 虚部が 0 でない場合
   */
  _requireRealPart(operation) {
    if (!this.isReal()) _BigFloatComplex._throwNonRealOperation(operation);
    return this._real.clone();
  }
  /**
   ...
   * @throws {RangeError} 精度が 0 未満または MAX_PRECISION を超える場合
   */
  _coerceRealOperand(other, operation) {
    const rhs = _BigFloatComplex._toComplex(other, this._precision);
    if (!rhs.isReal()) _BigFloatComplex._throwNonRealOperation(operation);
   ...
   * @throws {RangeError} 精度が 0 未満または MAX_PRECISION を超える場合
   */
  _applyRealUnaryComplex(operation, fn) {
    return _BigFloatComplex._fromRealResult(fn(this._requireRealPart(operation)));
  }
  /**
   ...
   * @throws {SyntaxError} 文字列が複素数表現として無効な場合
   */
  _applyRealBinaryComplex(other, operation, fn) {
    return _BigFloatComplex._fromRealResult(fn(this._requireRealPart(operation), this._coerceRealOperand(other, operation)));
  }
  /**
   * 実数専用の二項演算を適用する
   * @param other - 対象の値
   ...
   * @throws {SyntaxError} 文字列が複素数表現として無効な場合
   */
  _applyRealBinary(other, operation, fn) {
  ...
  mod(other) {
    const rhs = _BigFloatComplex._toComplex(other, this._precision);
    return _BigFloatComplex._fromBigFloats(this._requireRealPart("mod").mod(rhs._requireRealPart("mod")), this._imag.clone());
  }
  /**
   ...
   * @throws {CacheNotInitializedError} キャッシュが存在しない場合
   */
  static geometricMean(values, precision) {
    const arr = Array.from(values);
    if (arr.length === 0) return precision === void 0 ? this.zero() : this.zero(precision);
   ...
   * @throws {SyntaxError} 文字列が複素数表現として無効な場合
   */
  static harmonicMean(values, precision) {
    const arr = Array.from(values);
    if (arr.length === 0) return precision === void 0 ? this.zero() : this.zero(precision);
    const p = precision === void 0 ? BigFloat.DEFAULT_PRECISION : BigInt(precision);
    let sumRecip = this.zero(p);
    for (const val of arr) {
      sumRecip = sumRecip.add(_BigFloatComplex.from(val, p).reciprocal());
    }
    return _BigFloatComplex.from(arr.length, 0, p).div(sumRecip);
  }
  /**
   ...
   * @throwsSuppressed {DivisionByZeroError}
   */
  static rms(values, precision) {
    const arr = Array.from(values);
    if (arr.length === 0) return precision === void 0 ? this.zero() : this.zero(precision);
    const p = precision === void 0 ? BigFloat.DEFAULT_PRECISION : BigInt(precision);
    let sumSq = this.zero(p);
    for (const val of arr) {
      const bfc = _BigFloatComplex.from(val, p);
...
var BigFloat = class _BigFloat {
  ...
  static _roundManual(mantissa, divisor) {
    ...
    switch (mode) {
      ...
      case 3 /* FLOOR */:
        ...
        break;
      case 4 /* HALF_UP */:
        if (absRem << 1n >= divisor) offset = isNeg ? -1n : 1n;
        break;
      case 5 /* HALF_DOWN */:
        if (absRem << 1n > divisor) offset = isNeg ? -1n : 1n;
        break;
   ...
   * @throws {CacheNotInitializedError} キャッシュが存在しない場合
   */
  static eulerGamma(precision = this.DEFAULT_PRECISION) {
    const precisionBig = BigInt(precision);
    this._checkPrecision(precisionBig);
  ...
  static rms(...args) {
    ...
    for (const val of arr) {
      const bf = val instanceof _BigFloat ? val : new this(val);
  ...
  agm(other) {
    const construct = this.constructor;
    let a = this.abs();
    let b = other instanceof BigFloatComplex ? other.abs() : _BigFloat.abs(other, this._precision);
  ...
  Ei() {
    ...
    if (!this._isFiniteState()) {
      this._ensureSpecialValuesEnabled(this);
      if (this._specialState === 1 /* POSITIVE_INFINITY */) return this._specialResult(1 /* POSITIVE_INFINITY */);
      if (this._specialState === 2 /* NEGATIVE_INFINITY */) return construct.zero(this._precision);
      return this._specialResult(3 /* NAN */);
    }
    if (this.isZero()) return this._specialResult(2 /* NEGATIVE_INFINITY */);
    const totalPr = this._precision + construct.config.extraPrecision;
    const val = this._getInternalValue(totalPr);
   ...
   * @returns 存在する場合はtrue
   */
  static _getCheckEulerGammaCache(precision) {
   ...
   * @throws {CacheNotInitializedError} キャッシュが存在しない場合
   */
  static _getEulerGammaCache(precision) {
    const cachedData = this._eulerGammaCache;
    if (cachedData) {
      return this._rescaleInternalValue(cachedData.exactValue, cachedData.precision, precision);
    }
   ...
   * @param precision - 精度
   */
  static _updateEulerGammaCache(value, precision) {
    const cachedData = this._eulerGammaCache;
    if (cachedData && cachedData.precision >= precision) {
      return;
    }
  ...
  static _refinePiFromCache(seed, precision) {
    ...
    while (currentPrecision < precision) {
      const grownPrecision = currentPrecision > 0n ? currentPrecision << 1n : 1n;
...
var BigFloatMatrix = class _BigFloatMatrix {
   ...
   * @throws {DimensionMismatchError} 行列の行が同じ長さを持たない場合
   * @throws {RangeError} 精度が 0 未満または MAX_PRECISION を超える場合
   */
  ...
  static _toBigFloat(value, precision) {
    ...
    if (value instanceof BigFloatComplex) {
      const cloned = value.clone();
      if (precision === void 0 || cloned.precision === precision) return cloned;
      return cloned.changePrecision(precision);
    }
    return new BigFloat(value, precision ?? BigFloat.DEFAULT_PRECISION);
  }
  ...
  static _assertRectangularRaw(rows) {
    ...
    for (const row of rows) {
      if (row.length !== columnCount) throw new DimensionMismatchError("Matrix rows must have the same length");
    }
  }
  /**
   * 同形状か検証する
   * @throws {DimensionMismatchError} 行列の形状が異なる場合
   */
  static _assertSameShape(left, right) {
    if (left.rowCount !== right.rowCount || left.columnCount !== right.columnCount) {
      throw new DimensionMismatchError("Matrix shapes must match");
    }
  }
  /**
   * 正方行列か検証する
   * @throws {DimensionMismatchError} 行列が正方行列でない場合
   */
  static _assertSquare(matrix) {
    if (!matrix.isSquare()) throw new DimensionMismatchError("Matrix must be square");
  }
  /**
   * 行列積可能か検証する
   * @throws {DimensionMismatchError} 行列の内積次元が一致しない場合
   */
  static _assertMultipliable(left, right) {
    if (left.columnCount !== right.rowCount) throw new DimensionMismatchError("Inner matrix dimensions must agree");
  }
  /**
  ...
  _mapValues(fn) {
    const values = this._values.map(
      (currentRow, rowIndex) => currentRow.map((value, columnIndex) => {
        ...
        return mapped instanceof BigFloat || mapped instanceof BigFloatComplex ? mapped.clone() : _BigFloatMatrix._toBigFloat(mapped, value._precision);
      })
    );
  ...
  _mapWithOperand(other, fn) {
    if (other instanceof BigFloatComplexMatrix || BigFloat._isComplexValue(other)) {
      ...
      return BigFloatComplexMatrix.from(this.toArray()).zipMap(other, (l, r, row, col) => fn(l.real, r, row, col));
    }
   ...
   * @param precision - 精度
   * @returns BigFloatMatrix インスタンス
   * @throws {DimensionMismatchError} 列ベクトルの長さが異なる場合
   * @throws {TypeError} 複素数モードが無効な場合
   * @throws {SyntaxError} 文字列が複素数表現として無効な場合
   */
  static fromColumns(columns, precision) {
    ...
    for (const column of rawColumns) {
      if (column.length !== rowCount) throw new DimensionMismatchError("Matrix columns must have the same length");
    }
   ...
   * @param fn - 変換関数
   * @returns 変換後の新しい行列
   * @throws {DimensionMismatchError} 行列形状が一致しない場合
   * @throws {TypeError} 複素数モードが無効な場合
   * @throws {SyntaxError} 文字列が複素数表現として無効な場合
   * @throws {RangeError} 精度が 0 未満または MAX_PRECISION を超える場合
   */
   ...
   * @returns 連結された新しい行列
   * @throws {DimensionMismatchError} 列数が一致しない場合
   * @throws {TypeError} 複素数モードが無効な場合
   * @throws {SyntaxError} 文字列が複素数表現として無効な場合
   */
  concatRows(...others) {
    ...
    for (const other of others) {
      const matrix = _BigFloatMatrix._coerceMatrix(other, this._flattenValues());
      if (this.columnCount !== 0 && matrix.columnCount !== this.columnCount) throw new DimensionMismatchError("Column counts must match");
   ...
   * @param others - 連結する行列のリスト
   * @returns 連結された新しい行列
   * @throws {DimensionMismatchError} 行数が一致しない場合
   * @throws {TypeError} 複素数モードが無効な場合
   * @throws {SyntaxError} 文字列が複素数表現として無効な場合
   */
  concatColumns(...others) {
    ...
    for (const other of others) {
      ...
      if (result.rowCount !== matrix.rowCount) throw new DimensionMismatchError("Row counts must match");
   ...
   * @param other - 乗じる行列
   * @returns 行列積の結果
   * @throws {DimensionMismatchError} 内積次元が一致しない場合
   * @throws {SyntaxError} 文字列が複素数表現として無効な場合
   * @throws {PrecisionMismatchError} 精度の不一致が許容されていない場合
   * @throws {TypeError} 複素数モードが無効な場合
   * @throws {SpecialValuesDisabledError} 特殊値が無効な設定で特殊値を扱おうとした場合、または対象に特殊値が含まれる場合
   ...
   * @returns 演算結果のベクトル
   * @throws {DimensionMismatchError} 内部次元が一致しない場合
   * @throws {SpecialValuesDisabledError} 特殊値が無効な設定で特殊値を扱おうとした場合
   * @throws {TypeError} 複素数モードが無効な場合
   * @throws {PrecisionMismatchError} 精度の不一致が許容されていない場合
   * @throws {SyntaxError} 文字列が複素数表現として無効な場合
   ...
   * 逆行列を計算する
   * @returns 逆行列
   * @throws {DimensionMismatchError} 正方行列でない場合、または行列が特異な場合
   * @throws {TypeError} 複素数モードが無効な場合
   * @throws {PrecisionMismatchError} 精度の不一致が許容されていない場合
   * @throws {SpecialValuesDisabledError} 特殊値が無効な設定で特殊値を比較しようとした場合
   * @throws {DivisionByZeroError} ゼロ除算が発生した場合
   * @throws {SyntaxError} 文字列が複素数表現として無効な場合
   * @throws {RangeError} size が負または非有限の場合
   * @throws {SingularMatrixError} 例外が発生した場合
   */
  inverse() {
    _BigFloatMatrix._assertSquare(this);
   ...
   * @param rhs - 右辺ベクトル b
   * @returns 解ベクトル x
   * @throws {DimensionMismatchError} 行列が正方でない場合、ベクトル長が不一致な場合、または行列が特異な場合
   * @throws {TypeError} 複素数モードが無効な場合
   * @throws {PrecisionMismatchError} 精度の不一致が許容されていない場合
   * @throws {SpecialValuesDisabledError} 特殊値が無効な設定で特殊値を比較しようとした場合
   * @throws {DivisionByZeroError} ゼロ除算が発生した場合
  ...
  solveVector(rhs) {
    ...
    if (vector.length !== this.rowCount) throw new DimensionMismatchError("Right-hand side vector length must match row count");
   ...
   * @param rhs - 右辺行列 B
   * @returns 解行列 X
   * @throws {DimensionMismatchError} 行列が正方でない場合、行数が不一致な場合、または行列が特異な場合
   * @throws {TypeError} 複素数モードが無効な場合
   * @throws {PrecisionMismatchError} 精度の不一致が許容されていない場合
   * @throws {SpecialValuesDisabledError} 特殊値が無効な設定で特殊値を比較しようとした場合
   * @throws {DivisionByZeroError} ゼロ除算が発生した場合
  ...
  solveMatrix(rhs) {
    ...
    if (right.rowCount !== this.rowCount) throw new DimensionMismatchError("Right-hand side row count must match");
    const size = this.rowCount;
    ...
    for (let row = 0; row < size; row++) {
      for (let column = 0; column < size; column++) {
        ...
        if (values[row][column].absoluteDiff(expected).gt(epsilon)) throw new SingularMatrixError("Matrix is singular");
      }
    }
   ...
   * @returns 演算結果
   * @throws {DimensionMismatchError} 正方行列でない場合、または指数が整数でない場合
   * @throws {TypeError} 複素数モードが無効な場合
   * @throws {PrecisionMismatchError} 精度の不一致が許容されていない場合
   * @throws {SpecialValuesDisabledError} 特殊値が無効な設定で特殊値を比較しようとした場合
   * @throws {DivisionByZeroError} ゼロ除算が発生した場合
...
var BigFloatComplexMatrix = class _BigFloatComplexMatrix {
   ...
   * @throws {DimensionMismatchError} 各行の長さが一致しない場合
   * @throws {RangeError} 精度が 0 未満または MAX_PRECISION を超える場合
   */
  constructor(rows = [], precision) {
    const rawRows = Array.from(rows, (row) => Array.from(row));
